...
module.exports = {
    ...
    execute: async () => {
        ...
        for (let i = 0; i < self.config.max_runs; i++) {
            ...
            let axiosConfig = {};
            if (self.config.use_proxies) {
                axiosConfig.proxy = proxies[i % proxies.length]; // Use proxies in a round-robin manner
            }
            ...
            const requestPromise = axios.get(`https://www.roblox.com/user-sponsorship/${numOrientation}`, axiosConfig).then(async (res) => {
                ...
                const response = await axios({
                    ...
                    method: 'GET',
                    responseType: 'stream',
                    proxy: proxies[i % proxies.length], // Use proxies in a round-robin manner
                });
                ...
                // You can uncomment this section if you want to send the webhook
